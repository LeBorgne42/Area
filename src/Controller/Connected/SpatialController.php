...
class SpatialController extends AbstractController
{
    ...
    public function spatialAction(Request $request, Planet $usePlanet)
    {
        ...
        if ($form_spatialShip->isSubmitted() && $form_spatialShip->isValid()) {
            ...
            $workerLess = (2000 * $motherShip) + (20000 * $moonMaker) + (100 * $cargoI) + (200 * $cargoV) + (250 * $cargoX) + (10000 * $colonizer) + (100 * $recycleur) + (25 * $barge) + (2 * $hunterWar) + (10 * $corvetWar) + (2 * $hunter) + (50 * $fregate) + (2 * $hunterHeavy) + (10 * $corvet) + (10 * $corvetLaser) + (50 * $fregatePlasma) + (100 * $croiser) + (80 * $ironClad) + (400 * $destroyer);
